...
async function fetchData(lang: Locale) {
	...
	try {
		await getServices(acceptedLanguage, 0, 10, (response: ApiResponse<Service[]>) => {
			if (response?.Status) {
				services = response?.Data;
			}
		});

		await getWorkShop(acceptedLanguage, 0, 10, (response: ApiResponse<WorkShop[]>) => {
			if (response?.Status && response?.Code === 200) {
				workshops = response?.Data;
			}
		});
		await getProjectsByDate(acceptedLanguage, 0, 10, (response: ApiResponse<ProjectsByYear>) => {
			if (response?.Status) {
...
export default async function OurWorld({ params: { lang } }: { params: { lang: Locale } }) {
